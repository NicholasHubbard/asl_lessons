...
// decided is a item should be loaded from the api because it has been longer than the refresh time
export function shouldLoad(loadedAt, refreshAt = refreshTime) {
  // get the currect time in milliseconds
  const now = Date.now();
  // get the difference in time between now and when this object was loaded
  const timeSinceLastLoad = now - loadedAt;
  // if the time since loaded is larger that the refresh time
...
// given on object and id remove the key of the id from the object
